...
export interface ViewOptions {
  ...
  Target: string[];
  DataHook?: (rawData: FMex.Candle[], Options: ViewOptions, opt: any) => any;
}
...
export const EchartReset = (echart: echarts.ECharts) => {
...
export async function EchartSetData(echart: echarts.ECharts, candles: FMex.Candle[], Options: ViewOptions) {
  echart.setOption(getOption(candles, Options));
}

function getOption(rawData: FMex.Candle[], Options: ViewOptions) {
  const dates: string[] = [];
  const kData = {
    k: [] as number[][],
    v: [] as number[][],
  ...
  rawData.forEach((item, index) => {
    dates.push(moment(item.timestamp).format('YYYY-MM-DD hh:mm'));
