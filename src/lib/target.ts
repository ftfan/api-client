import { ViewOptions } from '@/lib/View';
...
export const TargetResults = (rawData: FMex.Candle[], Options: ViewOptions, opt: any) => {
...
function MA(dayCount: number, rawData: FMex.Candle[]) {
  const result = [];
...
function BOOL(maDays: number, rawData: FMex.Candle[]) {
  //移动平均线周期为20
...
function OBV(rawData: FMex.Candle[]) {
...
function OBV2(rawData: FMex.Candle[]) {
  let lastTick: any = null;
  const obvs = [];
  for (let i = 0; i < rawData.length; i++) {
    let value = 0;
...
function OBVNum(num: number, rawData: FMex.Candle[], Options: ViewOptions, opt: any) {
  let opacity = 1;
  const name = 'OBV';
...
function OBVNum2(num: number, rawData: FMex.Candle[], Options: ViewOptions, opt: any) {
  let opacity = 1;
...
function BOLLNum(num: number, rawData: FMex.Candle[], Options: ViewOptions, opt: any) {
  let opacity = 1;
  const name = 'BOLL' + num;
  if (Options.Target.indexOf(name) === -1) opacity = 0;
...
function MANum(num: number, rawData: FMex.Candle[], Options: ViewOptions, opt: any) {
  const name = 'MA' + num;
  let opacity = 1;
  if (Options.Target.indexOf(name) === -1) opacity = 0;
  opt.series.push({
